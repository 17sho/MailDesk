...
[![Version](https://img.shields.io/badge/version-0.4.0-2563eb.svg)](https://github.com/17sho/MailDesk/releases)
...
当前仓库版本为 **0.4.0**。Windows 与 macOS 使用同一正式版本和同一个 GitHub Release。发行版采用便携数据布局：数据库、密钥、邮件和日志保存在程序旁的 `MailDesk Data`，更新事务保存在同级 `.maildesk-update`，不会默认写到其他磁盘。密码、应用专用密码、Refresh Token、TOTP 密钥与代理密码等凭据字段使用 Windows DPAPI 或 macOS 钥匙串保护的随机 Fernet 主密钥进行加密。
...
在 [MailDesk v0.4.0 Release](https://github.com/17sho/MailDesk/releases/tag/v0.4.0) 下载带版本号的 Windows x64 压缩包：

| 文件 | 适用场景 |
|---|---|
| `MailDesk-v0.4.0-windows-x64-onefile.zip` | 解压后直接运行单个 `MailDesk.exe`，携带方便，首次启动较慢 |
| `MailDesk-v0.4.0-windows-x64-onedir.zip` | 保持目录完整并运行 `MailDesk\MailDesk.exe`，启动更快，推荐日常使用 |
| `MailDesk-v0.4.0-hotfix-windows-x64-onedir.zip` | 已安装 v0.3.9 文件夹版且自动更新回滚时，彻底退出程序后人工覆盖一次 `MailDesk` 程序文件夹；保留同级 `MailDesk Data` |
| `MailDesk-v0.3.9-onedir-update-launcher.exe` | 仅供已经安装的 v0.3.9 文件夹版修复一次升级启动目录；不包含账号数据，也不会修改 `MailDesk Data` |
...
macOS 与 Windows 共用 [v0.4.0 正式 Release](https://github.com/17sho/MailDesk/releases/tag/v0.4.0)。Apple Silicon 下载 `MailDesk-v0.4.0-macos-arm64.dmg`，Intel Mac 下载 `MailDesk-v0.4.0-macos-x64.dmg`；ZIP 是应用内自动更新包。架构选择、钥匙串存储、Gatekeeper 首次打开步骤和限制见 [`docs/MACOS.md`](docs/MACOS.md)。
...
Windows v0.3.9 文件夹版曾把安装助手启动在即将被替换的 `MailDesk` 目录中，Windows 会因此拒绝移动该目录并自动回滚。v0.4.0 热修复已让安装助手固定使用外层安全目录，并对短暂占用进行重试。已经装好的旧 v0.3.9 无法远程改写其 EXE 内置逻辑，推荐从 Release 下载一次 `MailDesk-v0.4.0-hotfix-windows-x64-onedir.zip`：彻底退出 MailDesk，只替换旧安装位置中的 `MailDesk` 程序文件夹，保留同级 `MailDesk Data`。5.5 KB 的 `MailDesk-v0.3.9-onedir-update-launcher.exe` 仅用于以正确工作目录重新打开旧版、复测原始升级按钮；它不下载、不替换、不读取账号数据，要安装根治代码仍应使用 hotfix ZIP。
...
.\.venv\Scripts\python.exe release.py --version 0.4.0 `
  --extra-asset .\macos\MailDesk-v0.4.0-macos-arm64.zip `
  --extra-asset .\macos\MailDesk-v0.4.0-macos-x64.zip `
  --extra-asset .\macos\MailDesk-v0.4.0-macos-arm64.dmg `
  --extra-asset .\macos\MailDesk-v0.4.0-macos-x64.dmg
