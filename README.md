...
[![Version](https://img.shields.io/badge/version-0.3.8-2563eb.svg)](https://github.com/17sho/MailDesk/releases)
...
当前仓库版本为 **0.3.8**。Windows 与 macOS 使用同一正式版本和同一个 GitHub Release。Windows 数据默认保存在 `%LOCALAPPDATA%\MailDesk`，macOS 数据保存在 `~/Library/Application Support/MailDesk`；密码、应用专用密码、Refresh Token、TOTP 密钥与代理密码等凭据字段使用 Windows DPAPI 或 macOS 钥匙串保护的随机 Fernet 主密钥进行加密。
...
| 批量取件 | ✅ | `QThreadPool` 并发、1–50 并发配置、IMAP 分批下载、连接/命令分级超时、瞬时超时安全重连、协作式停止与阶段化错误分类 |
...
在 [MailDesk v0.3.8 Release](https://github.com/17sho/MailDesk/releases/tag/v0.3.8) 下载带版本号的 Windows x64 压缩包：

| 文件 | 适用场景 |
|---|---|
| `MailDesk-v0.3.8-windows-x64-onefile.zip` | 解压后直接运行单个 `MailDesk.exe`，携带方便，首次启动较慢 |
| `MailDesk-v0.3.8-windows-x64-onedir.zip` | 保持目录完整并运行 `MailDesk\MailDesk.exe`，启动更快，推荐日常使用 |
...
macOS 与 Windows 共用 [v0.3.8 正式 Release](https://github.com/17sho/MailDesk/releases/tag/v0.3.8)。Apple Silicon 下载 `MailDesk-v0.3.8-macos-arm64.dmg`，Intel Mac 下载 `MailDesk-v0.3.8-macos-x64.dmg`；ZIP 是应用内自动更新包。架构选择、钥匙串存储、Gatekeeper 首次打开步骤和限制见 [`docs/MACOS.md`](docs/MACOS.md)。
...
- 更新事务使用跨进程文件锁，避免多个 MailDesk 实例争抢同一暂存区。临时更新文件和有限的安装结果记录保存在 `%LOCALAPPDATA%\MailDesk\updates` 或 `~/Library/Application Support/MailDesk/updates`。
...
macOS 对应目录为 `~/Library/Application Support/MailDesk`，主密钥保存于当前用户钥匙串，数据目录中只保留 `master.key.keychain` 标记文件。
...
.\.venv\Scripts\python.exe release.py --version 0.3.8 `
  --extra-asset .\macos\MailDesk-v0.3.8-macos-arm64.zip `
  --extra-asset .\macos\MailDesk-v0.3.8-macos-x64.zip `
  --extra-asset .\macos\MailDesk-v0.3.8-macos-arm64.dmg `
  --extra-asset .\macos\MailDesk-v0.3.8-macos-x64.dmg
