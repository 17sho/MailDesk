...
Windows v0.3.9 文件夹版曾把安装助手启动在即将被替换的 `MailDesk` 目录中，Windows 会因此拒绝移动该目录并自动回滚。v0.4.0 热修复已让安装助手固定使用外层安全目录，并对短暂占用进行重试。已经装好的旧 v0.3.9 无法远程改写其 EXE 内置逻辑，需要从 Release 下载一次 `MailDesk-v0.3.9-onedir-update-launcher.exe`，彻底退出 MailDesk 后运行它、选择旧版 `MailDesk.exe`，再回到软件点击“重启并安装”。该启动器只有约 5.5 KB，仅以正确工作目录重新打开旧版，不下载、不替换、不读取账号数据。
