...
[![Version](https://img.shields.io/badge/version-0.3.2-2563eb.svg)](https://github.com/17sho/MailDesk/releases)
...
当前仓库版本为 **0.3.2**。Windows 数据默认保存在 `%LOCALAPPDATA%\MailDesk`，macOS 数据保存在 `~/Library/Application Support/MailDesk`；密码、应用专用密码、Refresh Token、TOTP 密钥等凭据字段使用 Windows DPAPI 或 macOS 钥匙串保护的随机 Fernet 主密钥进行加密。
...
| macOS 打包 | 预览 | GitHub 原生 arm64/x64 Runner 构建 `.app`、DMG 与 ZIP；当前未做 Apple 公证 |
...
在 [MailDesk v0.3.2 Release](https://github.com/17sho/MailDesk/releases/tag/v0.3.2) 下载带版本号的 Windows x64 压缩包：

| 文件 | 适用场景 |
|---|---|
| `MailDesk-v0.3.2-windows-x64-onefile.zip` | 解压后直接运行单个 `MailDesk.exe`，携带方便，首次启动较慢 |
| `MailDesk-v0.3.2-windows-x64-onedir.zip` | 保持目录完整并运行 `MailDesk\MailDesk.exe`，启动更快，推荐日常使用 |
...
macOS 原生构建、架构选择、钥匙串存储、Gatekeeper 首次打开步骤和已知限制见 [`docs/MACOS.md`](docs/MACOS.md)。预览版 Release 同时提供 Apple Silicon `arm64` 与 Intel `x64` 的 DMG/ZIP，并附带 `SHA256SUMS-macos.txt`。
...
.\.venv\Scripts\python.exe release.py --version 0.3.2
...
- 生产凭据存储在 Windows 使用 DPAPI、在 macOS 使用当前用户钥匙串；macOS 目前仍为未公证预览版。
